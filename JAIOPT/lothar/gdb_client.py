...
from lothar import gdb_runner as gdbrunner
g_temp_dirs = []
...
def generate_gdb_script(root, sysroot, binary_name, port, dalvik_gdb_script, solib_search_path, connect_timeout):
    ...
    gdb_commands += 'python'
    gdb_commands += 'def target_remote_with_retry(target, timeout_seconds):'
    gdb_commands += '  import time'
    gdb_commands += '  end_time = time.time() + timeout_seconds'
    gdb_commands += '  while True:'
    gdb_commands += '    try:'
    gdb_commands += '      gdb.execute("target extended-remote " + target)'
    gdb_commands += '      return True'
    gdb_commands += '    except gdb.error as e:'
    gdb_commands += '      time_left = end_time - time.time()'
    gdb_commands += '      if time_left < 0 or time_left > timeout_seconds:'
    gdb_commands += '        print("Error: unable to connect to device.")'
    gdb_commands += '        print(e)'
    gdb_commands += '        return False'
    gdb_commands += '      time.sleep(min(0.25, time_left))'
    gdb_commands += "target_remote_with_retry(':{}', {})\nend".format(port, connect_timeout)
    return gdb_commands
