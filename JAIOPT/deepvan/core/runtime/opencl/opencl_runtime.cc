...
#include "deepvan/core/runtime/opencl/opencl_extension.h"
...
const char *kOpenCLPlatformInfoKey =
    "deepvan_opencl_precompiled_platform_info_key";
}  // namespace
...
OpenCLRuntime::OpenCLRuntime(
  ...
  const char *profiling = getenv("DEEPVAN_OPENCL_PROFILING");
  if (IsTuning() ||
  ...
  const char *out_of_range_check = getenv("OUT_OF_RANGE_CHECK");
...
bool OpenCLRuntime::BuildProgramFromCache(
  ...
  *program = cl::Program(context(), {device()}, {*content});
  ...
  LOG(INFO) << "Program from Cache: " << built_program_key;
  return true;
}
...
bool OpenCLRuntime::BuildProgramFromPrecompiledBinary(
  ...
  *program = cl::Program(context(), {device()}, {*content});
  ...
  LOG(INFO) << "Program from precompiled binary: " << built_program_key;
  return true;
}
...
bool OpenCLRuntime::BuildProgramFromSource(
  ...
  if (it_source != kEncryptedProgramMap.end()) {
    ...
    *program = cl::Program(context(), sources);
    ...
    // LOG(INFO) << "Program from source: " << built_program_key;
  }
  return true;
}
...
bool OpenCLRuntime::BuildProgram(const std::string &program_name,
  ...
  //   }
  // }
  bool ret = BuildProgramFromSource(program_name, built_program_key,
                                   build_options_str, program);
  return ret;
}

VanState OpenCLRuntime::BuildKernel(
    const std::string &program_name,
  ...
  VLOG(1) << DEBUG_GPU
          << "Build options: " << build_options_str;
