#ifndef DEEPVAN_UTILS_TUNER_H_
#define DEEPVAN_UTILS_TUNER_H_

// TODO@vgod Fix portability
#include <fcntl.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <unistd.h>
...
namespace deepvan {
typedef struct opencl_kernel_conf {

  opencl_kernel_conf(std::string kname, 
                     std::string pname,
                     index_t chblks,
                     index_t wblks,
                     index_t hblks):
  ...
  index_t channel_blocks;
  index_t width_blocks;
  index_t height_blocks;
} KernelConfiguration;

inline bool IsTuning() {
  const char *tuning = getenv("DEEPVAN_TUNING");
...
class Tuner {
 public:
  explicit Tuner(const std::string tuned_param_file_path = "",
    ...
    path_ = getenv("DEEPVAN_RUN_PARAMETER_PATH");
  ...
  RetType TuneOrRun(
      ...
      Timer *timer) {
    std::string obfucated_param_key = DEEPVAN_OBFUSCATE_SYMBOL(param_key);
