...
class BMNet {
 public:
  static BMNet *Get();
  VanState Run(DeviceType device,
                 float *exec_time);
 ...
 private:
  ...
  void AddIntArg(OperatorProto *op_def, const std::string &name, int value);
  void AddIntsArg(OperatorProto *op_def,
                  const std::string &name,
                  const std::vector<int> &values);
  void AddStringArg(OperatorProto *op_def,
                    const std::string &name,
                    const std::string &value);
  void AddFloatArg(OperatorProto *op_def, const std::string &name, float value);
...
void BMNet::AddIntArg(deepvan::OperatorProto *op_def,
                      const std::string &name,
                      int value) {
...
void BMNet::AddIntsArg(deepvan::OperatorProto *op_def,
                       const std::string &name,
                       const std::vector<int> &values) {
  auto arg = op_def->add_arg();
  arg->set_name(name);
...
void BMNet::AddStringArg(deepvan::OperatorProto *op_def,
                         const std::string &name,
                         const std::string &value) {
  auto arg = op_def->add_arg();
  arg->set_name(name);
...
void BMNet::AddFloatArg(deepvan::OperatorProto *op_def,
                        const std::string &name,
                        float value) {
  auto arg = op_def->add_arg();
  arg->set_name(name);
...
void BMNet::AddTensor(const std::string &name,
                      ...
                      int64_t offset,
                      int64_t data_size) {
  TensorProto *tensor_ptr = net_.add_tensors();
...
VanState BMNet::Run(DeviceType device,
                      float *exec_time) {
  std::lock_guard<std::mutex> lock(run_mutex_);
  VanState status;
  ModelExecutionConfig config(device);
  ...
  for (size_t i = 0; i < input_count; ++i) {
    ...
    inputs[input_names_[i]] = deepvan::TensorWrapper(input_shapes_[i], buffer_in);
  }
  ...
  for (size_t i = 0; i < output_count; ++i) {
    ...
    auto buffer_out = std::shared_ptr<float>(new float[output_size],
                                             std::default_delete<float[]>());
    outputs[output_names_[i]] = deepvan::TensorWrapper(output_shapes_[i], buffer_out);
  }
...
Capability GetCapability(DeviceType device_type, float cpu_float32_exec_time) {
  ...
  float exec_time;
  VanState status = net->Run(device_type, &exec_time);
  if (status == VanState::SUCCEED) {
