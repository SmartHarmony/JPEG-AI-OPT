...
__kernel void transform_matmul_weight(BUFFER_OUT_OF_RANGE_PARAMS
                                      GLOBAL_WORK_GROUP_SIZE_DIM2
                                      __global const IN_DATA_TYPE *input,
                                      __private const int input_offset,
                                      __global DATA_TYPE *output,
                                      __private const int width,
                                      __private const int height) {
...
#ifndef NON_UNIFORM_WORK_GROUP
  if (wb >= global_size_dim0 || hb >= global_size_dim1) {
    return;
  }
#endif
  int in_offset = input_offset + hb * width + wb;
  int out_offset = wb * height + hb;
...
__kernel void copy_data_type(BUFFER_OUT_OF_RANGE_PARAMS
                             __private const int global_size_dim0,
                             __global IN_DATA_TYPE *input,
                             __private const int input_offset,
                             __global IN_DATA_TYPE *output) {
  const int out_idx = get_global_id(0);
  ...
  vstore4(vload4(out_idx, input + input_offset), out_idx, output);
}
