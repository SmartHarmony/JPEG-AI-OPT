...
__kernel void eltwise_slice(OUT_OF_RANGE_PARAMS
                            GLOBAL_WORK_GROUP_SIZE_DIM3
                            __read_only image2d_t input0,
#if defined(INPUT_SCALAR)
                            __private const float value,
  ...
  const int pos = mad24(width_idx, global_size_dim0, chan_idx);
  DATA_TYPE4 in0 = READ_IMAGET(input0, SAMPLER, (int2)(pos, hb));
#if defined(INPUT_SCALAR)
  DATA_TYPE4 in1 = (DATA_TYPE4)(value, value, value, value);
#elif defined(INPUT_VECTOR)
...
__kernel void eltwise_bert(OUT_OF_RANGE_PARAMS
                           ...
                           __read_only image2d_t input1,
#endif
#if defined(INPUT_BCSECOND_TENSOR)
                           __private const int height,
#endif
#ifdef COEFF_SUM
                           __private const float coeff0,
...
#ifndef NON_UNIFORM_WORK_GROUP
  if (width_idx >= global_size_dim0 || hb >= global_size_dim1)
    return;
#endif

  DATA_TYPE4 in0 = READ_IMAGET(input0, SAMPLER, (int2)(width_idx, hb));
#if defined(INPUT_SCALAR)
  DATA_TYPE4 in1 = (DATA_TYPE4)(value, value, value, value);
#elif defined(INPUT_SCALAR_TENSOR)
  DATA_TYPE4 tmp = READ_IMAGET(input1, SAMPLER, (int2)(0, 0));
  DATA_TYPE4 in1 = (DATA_TYPE4)(tmp.x, tmp.x, tmp.x, tmp.x);
#elif defined(INPUT_BCLAST_TENSOR)
  DATA_TYPE4 in1 = READ_IMAGET(input1, SAMPLER, (int2)(width_idx, 0));
#elif defined(INPUT_BCSECOND_TENSOR)
  // const int batch_idx = hb % height;
  DATA_TYPE4 in1_ = READ_IMAGET(input1, SAMPLER, (int2)(0, hb));
  DATA_TYPE4 in1 = (DATA_TYPE4){in1_.x, in1_.x, in1_.x, in1_.x};
#else
  DATA_TYPE4 in1 = READ_IMAGET(input1, SAMPLER, (int2)(width_idx, hb));
#endif
  ...
  out = diff * diff;
    ...
    out = floor(in0 / in1);
  #endif
#elif ELTWISE_TYPE == 14
  out = exp(in0);
#elif ELTWISE_TYPE == 15
  out = erf(in0);
#endif

  WRITE_IMAGET(output, (int2)(width_idx, hb), out);
}